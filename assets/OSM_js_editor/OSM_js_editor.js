...
function init_form_from_OSM(form,OSM_type,OSM_id) { 
    ...
    //Disable all radios buttons (wlan)
    $("[name=edit_form] .btn-group .btn").removeClass("active");
    //tout le reste
    ...
    for (i=0; i<n; i++)
    {
        if (inputForm[i].type.toLowerCase() === "checkbox" && inputForm[i].name === "beer")  // si c'est une case à cocher de type beer
        {
            if (inputForm[i].checked)
                inputForm[i].checked = false; //décocher tout par défaut
        ...
        if (inputForm[i].type.toLowerCase() === "radio" && inputForm[i].name === "wifi")  
        {
            switch(wifi_src)
            {
                ...
                case "yes":
                    ...
                    {
                        inputForm[i].checked = true;
                        inputForm[i].parentNode.className += " active";
                    }
                    break;
                case "non_fourni":
                    if (inputForm[i].value === "chaispas")
                    {
                        inputForm[i].checked = true;
                        inputForm[i].parentNode.className += " active";
                    }
...
function complete_form_with_already_maped_beers(beer_src){
    ...
    {
        if (!(x[i] in lcBeerName))
        {// ne pas ajouter les éléments déjà ajoutés par localstorage
            //console.log(x[i])
            if (x[i] != "non_fourni")
            {
